...
const TABLE_HEAD = ["No.", "Nama Mata Pelajaran"];
const ListMapel = ({ data }) => {
  const route = useRouter();
  const { showConfirmation, hideConfirmation } = useConfirmation();
  const [TABLE_ROWS, setTABLE_ROWS] = useState(data);
  const [isSuccess, setIsSuccess] = useState(null);

  const deleteMapel = async (mapelid) => {
    setIsSuccess(null);
    try {
      const res = await Axios.delete("/mapels/" + mapelid, {
        headers: {
        ...
        <div className="mb-6 flex md:items-center flex-col md:flex-row justify-between gap-8">
          ...
          <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
            <Button
              ...
              size="sm"
              onClick={() => route.push(`/admin/upload/mapel`)}
            >
              <UserPlusIcon strokeWidth={2} className="h-4 w-4" /> Tambah Mapel
            </Button>
          </div>
        </div>
        {isSuccess ? (
                    ...
                    <td className={classes + " text-center"}>
                      <Tooltip content="Edit">
                        <IconButton
                          variant="text"
                          onClick={() =>
                            route.push("/admin/update/mapel/" + id)
                          }
                        >
